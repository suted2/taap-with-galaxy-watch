...
def response(flow) -> None:  # flow: mitmproxy.http.HTTPFlow (런타임에 주입)
    ...
    # QR + court-auth(로그인/토큰) 흐름 전체를 덤프 (정적 자산 제외)
    dump_this = "/api/court/ac/access/qr" in path or "/api/court-auth/" in path
    if dump_this and not path.lower().endswith(_STATIC):
        import os
